<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:variable name="presults" select="count(//response/lst[@name='highlighting']//lst)"/>
  <xsl:variable name="tresults" select="/response/result/@numFound"/>
  <xsl:variable name="rows" select="//lst[@name='params']/str[@name='rows']"/>
  <xsl:variable name="navcount" select="floor( ( $tresults - $start ) div $rows)"/>
  ...
  <xsl:template name="navigator">
    <p>
      ...
      <xsl:if test="(response/result/@start)+(count(//response/lst[@name='highlighting']//lst)) &lt; response/result/@numFound">
        <span>
      ...
      <body>
        <xsl:if test="$tresults &gt; 0">
          <p>
            ...
            to
            <xsl:value-of select="(response/result/@start)+(count(//response/lst[@name='highlighting']//lst))"/>
            of
            <xsl:value-of select="response/result/@numFound"/>
          </p>
        </xsl:if>
